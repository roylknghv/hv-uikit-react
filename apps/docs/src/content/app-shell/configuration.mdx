...
3. The `$app/` prefix (e.g. `$app/pages/Hello.js`), referring to a module within the current Application Bundle.

> **Note:** The `@self/` prefix (e.g. `@self/pages/Hello.js`) is **deprecated** in favour of `$app/`. It is still supported for backwards compatibility but will be removed in a future major release.
